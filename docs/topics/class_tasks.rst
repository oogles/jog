...
.. note::
    ...
    Only output generated by the *task itself* (using its :attr:`~Task.stdout` and :attr:`~Task.stderr` attributes) is affected by the ``--stdout`` and ``--stderr`` arguments. Any output generated by ``jogger`` (such as an error message that the given task is invalid) will NOT be affected.
